...
They are different from [napari-managed plugin worker environments](managed-worker-environments), which a plugin declares for worker commands that require packages not supplied by napari.
